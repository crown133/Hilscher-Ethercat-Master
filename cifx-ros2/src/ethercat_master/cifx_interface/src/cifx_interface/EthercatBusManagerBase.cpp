...
//  anydrive
#include "cifx_interface/EthercatBusManagerBase.hpp"

namespace cifx_interface {

bool EthercatBusManagerBase::addEthercatBus(cifx_interface::EthercatBusBase* bus) {
  if (bus == nullptr) {
  ...
  if (it == buses_.end()) {
    buses_.insert(std::make_pair(bus->getName(), std::unique_ptr<cifx_interface::EthercatBusBase>(bus)));
    return true;
  } else {
    return false;
  }
...
bool EthercatBusManagerBase::addEthercatBus(std::unique_ptr<cifx_interface::EthercatBusBase> bus) {
  if (bus == nullptr) {
    MELO_ERROR_STREAM("[RokubiminiEthercatBusManager::addEthercatBus] bus is nullptr")
    return false;
  }
