  ...
  switch (configuration_.rxPdoTypeEnum) {
    case RxPdoTypeEnum::RxPdoStandard: {
      RxPdoStandard rxPdo{};
      {
        ...
        // std::cout << "RxPdoStandard controlWord_:" << rxPdo.controlWord_ << std::endl;
      }
...
bool Elmo::mapPdos(RxPdoTypeEnum rxPdoTypeEnum, TxPdoTypeEnum txPdoTypeEnum) {
  ...
  switch (rxPdoTypeEnum) {
    ...
    case RxPdoTypeEnum::RxPdoCST:
      ...
      std::this_thread::sleep_for(std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));
          sdoVerifyWrite(OD_INDEX_RX_PDO_ASSIGNMENT, 3, false, static_cast<uint16_t>(0x1615), configuration_.configRunSdoVerifyTimeout);
      std::this_thread::sleep_for(std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));
      rxSuccess &= sdoVerifyWrite(OD_INDEX_RX_PDO_ASSIGNMENT, 0, false, static_cast<uint8_t>(3), configuration_.configRunSdoVerifyTimeout);
      std::this_thread::sleep_for(std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));
      break;
  ...
  switch (txPdoTypeEnum) {
    ...
    case TxPdoTypeEnum::TxPdoCST:
      ...
      std::this_thread::sleep_for(std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));
      txSuccess &=
          sdoVerifyWrite(OD_INDEX_TX_PDO_ASSIGNMENT, 3, false, static_cast<uint16_t>(0x1a1E), configuration_.configRunSdoVerifyTimeout);
      std::this_thread::sleep_for(std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));     
      txSuccess &= sdoVerifyWrite(OD_INDEX_TX_PDO_ASSIGNMENT, 0, false, static_cast<uint8_t>(3), configuration_.configRunSdoVerifyTimeout);
