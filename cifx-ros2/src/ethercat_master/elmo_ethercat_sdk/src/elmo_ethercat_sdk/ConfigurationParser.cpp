...
void ConfigurationParser::parseConfiguration(YAML::Node configNode) {
  ...
  if (configNode["Hardware"].IsDefined()) {
    ...
    int32_t auxiliaryEncoderResolution;
    if (getValueFromFile(hardwareNode, "auxiliary_encoder_resolution", auxiliaryEncoderResolution)) {
      configuration_.auxiliaryEncoderResolution = auxiliaryEncoderResolution;
    }
